...
func init() {
	...
	flag.StringVar(
		...
		"push-endpoint",
		"",
		"WebSocket endpoint where to push statuses(and logs if logs endpoint is not set)",
	)
	flag.StringVar(
		&PushLogsEndpoint,
		"push-logs-endpoint",
		"",
		"WebSocket endpoint where to push logs(push-endpoint value is used by default)",
	)
	flag.StringVar(
		&runtimeIDRaw,
		"runtime-id",
	...
	Once servers for one installer available checkers stops`,
	)
}
